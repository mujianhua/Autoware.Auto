...
: Node("lateral_controller", node_options), m_tf_listener(m_tf_buffer, this, false)
{
  ...
  m_sub_steering = create_subscription<autoware_auto_vehicle_msgs::msg::VehicleKinematicState>(
    "input/current_kinematic_state", rclcpp::QoS{1},
    std::bind(&LateralController::onState, this, _1));
...
bool8_t LateralController::checkData() const
{
  ...
  if (!m_current_state_ptr) {
    RCLCPP_DEBUG(get_logger(), "waiting data. current_state = %d", m_current_state_ptr != nullptr);
    return false;
  }
  ...
  const autoware_auto_planning_msgs::msg::Trajectory::SharedPtr msg)
{
  ...
  m_mpc.setReferenceTrajectory(
    *m_current_trajectory_ptr, m_traj_resample_dist, m_enable_path_smoothing,
    m_path_filter_moving_ave_num, m_enable_yaw_recalculation, m_curvature_smoothing_num,
    m_current_pose_ptr);
}
...
bool8_t LateralController::updateCurrentPose()
{
  ...
  try {
    ...
    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 5000 /*ms*/, ex.what());
    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 5000 /*ms*/, m_tf_buffer.allFramesAsString());
    return false;
  }
...
void LateralController::declareMPCparameters()
{
  ...
  m_mpc.m_param.weight_heading_error_squared_vel =
    declare_parameter("mpc_weight_heading_error_squared_vel").get<float64_t>();
  ...
  m_mpc.m_param.weight_steering_input_squared_vel =
    declare_parameter("mpc_weight_steering_input_squared_vel").get<float64_t>();
  ...
  m_mpc.m_param.low_curvature_weight_lat_error =
    declare_parameter("mpc_low_curvature_weight_lat_error").get<float64_t>();
  m_mpc.m_param.low_curvature_weight_heading_error =
    declare_parameter("mpc_low_curvature_weight_heading_error").get<float64_t>();
  m_mpc.m_param.low_curvature_weight_heading_error_squared_vel =
    declare_parameter("mpc_low_curvature_weight_heading_error_squared_vel").get<float64_t>();
  m_mpc.m_param.low_curvature_weight_steering_input =
    declare_parameter("mpc_low_curvature_weight_steering_input").get<float64_t>();
  m_mpc.m_param.low_curvature_weight_steering_input_squared_vel =
    declare_parameter("mpc_low_curvature_weight_steering_input_squared_vel").get<float64_t>();
  m_mpc.m_param.low_curvature_weight_lat_jerk =
    declare_parameter("mpc_low_curvature_weight_lat_jerk").get<float64_t>();
  m_mpc.m_param.low_curvature_weight_steer_rate =
    declare_parameter("mpc_low_curvature_weight_steer_rate").get<float64_t>();
  m_mpc.m_param.low_curvature_weight_steer_acc =
    declare_parameter("mpc_low_curvature_weight_steer_acc").get<float64_t>();
  m_mpc.m_param.low_curvature_thresh_curvature =
    declare_parameter("mpc_low_curvature_thresh_curvature").get<float64_t>();
  ...
  m_mpc.m_param.weight_terminal_heading_error =
    declare_parameter("mpc_weight_terminal_heading_error").get<float64_t>();
